...
flowPaths, flowPathways, flowList = getRecursiveFlow(startValve, 26, valvesWithFlow, [], valveList, 0,flowPaths,flowPathways,[], flowList)


highestFlowIndices = [0,0]
flowRank = rank(2)

for i, flowNum in enumerate(flowPaths):
    flowRank.insert(flowNum)

for i,flow in enumerate(flowPaths):
    for j,rankerFlow in enumerate(flowRank.rankings):
        if rankerFlow == flow:
            highestFlowIndices[j] = i
            break



for bestFlowPathIndices in highestFlowIndices:
    timeLeft = 26
    flow = 0
    currentValve = startValve
    for i, valve in enumerate(flowPathways[bestFlowPathIndices]):
        valveFlow, timeLeft = findSummedFlowOfValve(currentValve, valve, valveList, timeLeft)
        currentValve = valve
        flow += valveFlow
        print("opened valve", valve.name, "and got", flow, "and time left:", timeLeft, "and recursive current flow", flowList[bestFlowPathIndices][i])
