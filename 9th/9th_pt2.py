...
class RopeSim:
    ...
    def setHeadAndTail(self, headToCoord: Coords):
        self.head = headToCoord
        diff = self.tail - self.head
        absMaxCoordVal = abs(diff).max() #returns the highest coordinate in positive numbers
        if absMaxCoordVal > 1:
...
ropeBody = [RopeSim()]
for i in range(8):
    ropeBody.append(RopeSim(ropeBody[i]))
...
for move in listOfMoves:
    ...
    for i in range(int(move[1])):
        ropeBody[0].move(move[0])

nonDuplicatedListOfMoves = []
[nonDuplicatedListOfMoves.append(moveCoord) for moveCoord in ropeBody[-1].tailMovedPos if moveCoord not in nonDuplicatedListOfMoves]
