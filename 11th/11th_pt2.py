...
class Monkey:
    ...
    def playTurn(self, monkeyList, monkeyModMultiplier):
        for item in self.items:
            self.activity += 1
            item = self.operationLambda(item)

            item -= math.floor(item/monkeyModMultiplier)*monkeyModMultiplier

            #print("item: {} mod {} = {}".format(item, self.testCondition, item%self.testCondition))
...
monkeyModMultiplier = 1
for monkey in monkeyData:
    monkeyModMultiplier *= monkey.testCondition

for i in range(10000):
    for monkey in monkeyData:
        monkey.playTurn(monkeyData, monkeyModMultiplier)

    if (i + 1) % 1000 == 0:
        print("round {} activity".format(i+1))
        for monkey in monkeyData:
            print(monkey.activity) 
        print("round {} activity end".format(i+1))
