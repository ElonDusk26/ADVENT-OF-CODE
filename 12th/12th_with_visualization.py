...
def convertMazeToDatapoints(inputMaze: maze):
    out = []
    for row in inputMaze.maze:
        toAppend = []
        for mazePoint in row:
            mazeVal = int(ord(mazePoint)-97)
            toAppend.append(mazeVal)
        out.append(toAppend)
    return out
...
def floodGateSolve(legalMoves, visitedPoints, inputMaze: maze, depthCounter):
    ...
    visitedPlot = [list(map(int, visitedPointsRow)) for visitedPointsRow in visitedPoints]
    fig = plt.figure()
    ax = fig.add_subplot(111)
    plotPoints = convertMazeToDatapoints(inputMaze)
    plotPoints[inputMaze.end.row][inputMaze.end.column] = 30

    ax.imshow(plotPoints, aspect='auto', interpolation='nearest')
    ax.imshow(visitedPlot, aspect='auto', interpolation='nearest', cmap=cmap)
    plt.pause(.01)
    plt.close(fig=fig)
